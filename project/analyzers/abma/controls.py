...
from .events import _ensure_columns, _ensure_tz_aware, REQUIRED_CALENDAR_COLUMNS, REQUIRED_QUOTES_COLUMNS, REQUIRED_TRADES_COLUMNS
...
def _compute_baseline_metrics(
    ...
    *,
    ...
    start = anchor_ts - pd.Timedelta(minutes=config.baseline_window_minutes)
    if start < session_open_ts:
        return {"baseline_volume": np.nan, "baseline_rv": np.nan, "baseline_valid": False}
    trades_win = symbol_trades[(symbol_trades["ts"] >= start) & (symbol_trades["ts"] < anchor_ts)]
    volume = float(trades_win["size"].astype(float).sum()) if not trades_win.empty else 0.0

    grid = pd.date_range(start=start, end=anchor_ts - pd.Timedelta(seconds=1), freq=f"{config.baseline_grid_seconds}s", tz="UTC")
    if grid.empty:
        return {"baseline_volume": volume, "baseline_rv": np.nan, "baseline_valid": False}
    grid_df = pd.DataFrame({"ts": grid})
    asof_quotes = pd.merge_asof(
        grid_df,
        symbol_quotes.sort_values("ts"),
        on="ts",
        direction="backward",
    )
    mid = (asof_quotes["bid_px"].astype(float) + asof_quotes["ask_px"].astype(float)) / 2.0
    mid = mid.where(mid > 0.0)
    valid_mid = mid.dropna()
    if len(valid_mid) < config.min_valid_midpoints:
        return {"baseline_volume": volume, "baseline_rv": np.nan, "baseline_valid": False}

    log_mid = np.log(valid_mid)
    rv = float(log_mid.diff().pow(2).dropna().sum())
    return {"baseline_volume": volume, "baseline_rv": rv, "baseline_valid": np.isfinite(rv)}
...
def _precompute_baselines(
    ...
    *,
    ...
    # Enforce v1 boundary rule: invalidate anchors whose [t-15m, t) crosses session open.
    out["baseline_valid"] = out["baseline_valid"] & (out["anchor_ts"] >= open_ts + pd.Timedelta(minutes=config.baseline_window_minutes))
    return out


def _anchors_for_session(open_ts: pd.Timestamp, close_ts: pd.Timestamp, *, config: ABMAConfig) -> pd.DatetimeIndex:
...
def sample_matched_controls(
    ...
    *,
    ...
    _ensure_columns(events, {"event_id", "symbol", "session_date", "t0", "open_ts_utc", "close_ts_utc"}, "events")
    ...
    events_local["t0"] = _ensure_tz_aware(events_local["t0"], "events.t0")
    events_local["open_ts_utc"] = _ensure_tz_aware(events_local["open_ts_utc"], "events.open_ts_utc")
    events_local["close_ts_utc"] = _ensure_tz_aware(events_local["close_ts_utc"], "events.close_ts_utc")
    ...
    # Normalize to a deterministic order in case upstream feeds are slightly out-of-order.
    ...
    for (symbol, session_date), group in grouped_events:
        ...
        for _, event_row in group.iterrows():
            ...
            non_auction = candidate_df[candidate_df["anchor_ts"] != t0].copy()
            exact_pool = non_auction[(non_auction["rv_q"] == event_rv_q) & (non_auction["vol_q"] == event_vol_q)]
            match_tier = "exact"
            pool = exact_pool
            if len(pool) < config.control_set_size:
                pool = non_auction[non_auction["rv_q"] == event_rv_q].copy()
                match_tier = "rv_only"
            if pool.empty:
                no_controls.append(
                    {
