...
def main() -> int:
    parser = argparse.ArgumentParser(description="Ingest Binance spot OHLCV 15m from archives")
    ...
    manifest = start_manifest("ingest_binance_spot_ohlcv_15m", run_id, params, inputs, outputs)
        ...
        for symbol in symbols:
            ...
            for month_start in _iter_months(effective_start, effective_end):
                ...
                monthly_url = join_url(
                    ...
                    symbol,
                    "15m",
                    f"{symbol}-15m-{month_start.year}-{month_start.month:02d}.zip",
                )
                    ...
                    if result.status == "ok":
                        ...
                        for day in _iter_days(range_start, range_end_exclusive - timedelta(seconds=1)):
                            daily_url = join_url(
                                ...
                                symbol,
                                "15m",
                                f"{symbol}-15m-{day.year}-{day.month:02d}-{day.day:02d}.zip",
                            )
