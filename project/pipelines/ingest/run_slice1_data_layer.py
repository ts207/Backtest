...
def main():
    ...
    # 1. Ingestion (Raw)
    run_script("project/pipelines/ingest/ingest_binance_um_ohlcv_5m.py", common_args + time_args)
    run_script("project/pipelines/ingest/ingest_binance_spot_ohlcv_5m.py", common_args + time_args)
    run_script("project/pipelines/ingest/ingest_binance_um_mark_price_5m.py", common_args + time_args)
    ...
    # 2. Cleaning (Cleaned)
    run_script("project/pipelines/clean/build_cleaned_5m.py", common_args + ["--market", "perp", "--force", str(args.force)])
    run_script("project/pipelines/clean/build_cleaned_5m.py", common_args + ["--market", "spot", "--force", str(args.force)])
