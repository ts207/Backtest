...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase-1 ABMA analyzer pipeline")
    ...
    out_dir = Path(args.out_dir) if args.out_dir else DATA_ROOT / "reports" / "abma" / args.run_id
    ensure_dir(out_dir)
    ...
    if trades.empty or quotes.empty or session_calendar.empty:
        raise ValueError("Required inputs are empty after symbol/date filtering")

    events = extract_auction_boundary_events(trades=trades, quotes=quotes, session_calendar=session_calendar, config=DEFAULT_ABMA_CONFIG)
    if events.empty:
        raise ValueError("No ABMA events extracted")
    ...
    eval_outputs = evaluate_stabilization(event_metrics=merged_event_metrics, curves=metrics["curves"], config=DEFAULT_ABMA_CONFIG)
    report_payload = build_report_artifacts(eval_outputs, config=DEFAULT_ABMA_CONFIG)
    ...
    # Keep the schema outputs explicit and inspectable.
    ...
    print(f"ABMA v1 completed: {out_dir}")
