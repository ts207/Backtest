...
def _strategy_returns(
    ...
    strategy_metadata = positions.attrs.get("strategy_metadata", {})
    dsl_diagnostics = positions.attrs.get("diagnostics", {}) if hasattr(positions, "attrs") else {} if hasattr(positions, "attrs") else {}
    ...
    if "base_fee_bps" not in execution_cfg and "base_slippage_bps" not in execution_cfg:
        # Legacy fallback: treat --cost_bps as an all-in per-turnover cost.
        # Keep fee at 0 so cost is not silently split into fee vs slippage.
        execution_cfg["base_fee_bps"] = 0.0
        execution_cfg["base_slippage_bps"] = float(cost_bps)
    else:
        execution_cfg.setdefault("base_fee_bps", 0.0)
        execution_cfg.setdefault("base_slippage_bps", 0.0)
...
def _assert_next_bar_execution(pos_series):
    # Position changes must not claim same-bar pnl realization.
    # Ensures strategy timing respects shift(1) PnL model.
    changes = pos_series.diff().fillna(0) != 0
    # No hard failure here; mark diagnostic
    if changes.any():
        pass  # placeholder for stricter enforcement in future