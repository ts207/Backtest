...
from pipelines._lib.io_utils import list_parquet_files, read_parquet, write_parquet, ensure_dir
...
def _gap_lengths(is_gap: pd.Series) -> pd.Series:
    """
    Compute contiguous gap lengths in a boolean Series.
    """
...
def _align_funding(bars: pd.DataFrame, funding: pd.DataFrame) -> pd.DataFrame:
    """
    Align funding rates to bar timestamps, forward-filling for up to 96 bars.
    """
    if funding.empty:
        bars = bars.copy()
...
def main() -> int:
    ...
    try:
        for symbol in symbols:
            ...
            full_index = pd.date_range(start=start_ts, end=end_ts, freq="15min", tz=timezone.utc)
