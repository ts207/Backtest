...
import pipelines.research.analyze_trend_structure_events as analyzer
    ...
    return pd.DataFrame(
        {
            ...
            "high_96": pd.Series(high).rolling(96, min_periods=16).max(),
            "low_96": pd.Series(low).rolling(96, min_periods=16).min(),
        }
    )
...
def test_detect_mask_returns_boolean_series_for_supported_event():
    df = _sample_features()
    mask = analyzer._event_mask(df, event_type="RANGE_BREAKOUT")
...
def test_main_writes_rows_for_requested_event_type(monkeypatch, tmp_path):
    monkeypatch.setattr(
        ...
        {
            "RANGE_BREAKOUT": SimpleNamespace(
                reports_dir="trend_structure",
                events_file="trend_structure_events.csv",
            )
        },
    )
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "analyze_trend_structure_events.py",
            "--run_id",
            "r_test",
            "--symbols",
            "BTCUSDT",
    ...
    rc = analyzer.main()
    assert rc == 0
    out_csv = tmp_path / "out" / "trend_structure_events.csv"
    assert out_csv.exists()
    out = pd.read_csv(out_csv)
    if not out.empty:
        assert set(out["event_type"].astype(str).unique()) == {"RANGE_BREAKOUT"}
