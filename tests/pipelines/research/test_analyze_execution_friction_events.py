...
import pipelines.research.analyze_execution_friction_events as analyzer
...
def test_detect_mask_returns_boolean_series_for_supported_event():
    df = _sample_features()
    mask = analyzer._event_mask(df, event_type="SPREAD_REGIME_WIDENING_EVENT")
...
def test_main_writes_rows_for_requested_event_type(monkeypatch, tmp_path):
    monkeypatch.setattr(
        ...
        {
            "SPREAD_REGIME_WIDENING_EVENT": SimpleNamespace(
                reports_dir="execution_friction",
                events_file="execution_friction_events.csv",
            )
        },
    )
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "analyze_execution_friction_events.py",
            "--run_id",
            "r_test",
            "--symbols",
            "BTCUSDT",
    ...
    rc = analyzer.main()
    assert rc == 0
    out_csv = tmp_path / "out" / "execution_friction_events.csv"
    assert out_csv.exists()
    out = pd.read_csv(out_csv)
    if not out.empty:
        assert set(out["event_type"].astype(str).unique()) == {"SPREAD_REGIME_WIDENING_EVENT"}
