...
def test_load_phase1_events_uses_registry_spec_paths_and_subtype_filter(monkeypatch, tmp_path):
    ...
    pd.DataFrame(
        [
            ...
            {"event_type": "funding_acceleration", "symbol": "BTCUSDT", "enter_ts": "2026-01-01T00:05:00Z"},
        ]
    ).to_csv(events_path, index=False)

    out = evaluate_naive_entry._load_phase1_events(run_id=run_id, event_type="funding_acceleration")
    assert not out.empty
    assert set(out["event_type"].astype(str).unique()) == {"funding_acceleration"}
