...
def test_state_allowed_templates_enforced(tmp_path):
    run_id = "r_state_templates"
    atlas_dir = tmp_path / "atlas"
    atlas_dir.mkdir()
        ...
        [
            {
                ...
                "ontology_spec_hash": "sha256:test",
                "target_spec_path": "spec/events/CROSS_VENUE_DESYNC.yaml",
                "rule_templates": ["desync_repair", "convergence"],
                "horizons": ["5m"],
                "conditioning": {"vol_regime": []},
                "assets_filter": "*",
                "min_events": 50,
        ...
        "states": [
            {
                ...
                "min_events": 100,
                "activation_rule": "a",
                "allowed_templates": ["desync_repair"],
            },
            {
                "state_id": "F1",
                "family": "INFORMATION_DESYNC",
    ...
    assert rows
    s1_rows = [row for row in rows if row.get("state_id") == "S1"]
    f1_rows = [row for row in rows if row.get("state_id") == "F1"]
    assert s1_rows and f1_rows
    assert {row["rule_template"] for row in s1_rows} == {"desync_repair"}
    assert {row["rule_template"] for row in f1_rows} == {"convergence"}
