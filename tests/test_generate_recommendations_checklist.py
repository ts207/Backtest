...
def test_checklist_promote_when_all_gates_pass(monkeypatch, tmp_path: Path) -> None:
    ...
    edge_path.write_text(
        "\n".join(
            [
                "run_id,event,candidate_id,status,edge_score,expected_return_proxy,stability_proxy,n_events,gate_bridge_tradable,source_path",
                f"{run_id},vol_shock_relaxation,c1,PROMOTED,1.2,0.1,0.8,210,1,x",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def test_checklist_keeps_research_when_bridge_tradable_missing(monkeypatch, tmp_path: Path) -> None:
    run_id = "run_no_bridge_tradable"
    reports_root = tmp_path / "reports"
    edge_path = reports_root / "edge_candidates" / run_id / "edge_candidates_normalized.csv"
    edge_path.parent.mkdir(parents=True, exist_ok=True)
    edge_path.write_text(
        "\n".join(
            [
                "run_id,event,candidate_id,status,edge_score,expected_return_proxy,stability_proxy,n_events,gate_bridge_tradable,source_path",
                f"{run_id},vol_shock_relaxation,c1,PROMOTED,1.2,0.1,0.8,210,0,x",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    payload = json.loads((out_dir / "checklist.json").read_text(encoding="utf-8"))
    assert payload["decision"] == "KEEP_RESEARCH"
    assert any("bridge-tradable candidates below threshold" in reason for reason in payload["failure_reasons"])


def test_checklist_treats_promoted_research_as_promoted(monkeypatch, tmp_path: Path) -> None:
    run_id = "run_promoted_research"
    reports_root = tmp_path / "reports"
    edge_path = reports_root / "edge_candidates" / run_id / "edge_candidates_normalized.csv"
    edge_path.parent.mkdir(parents=True, exist_ok=True)
    edge_path.write_text(
        "\n".join(
            [
                "run_id,event,candidate_id,status,edge_score,expected_return_proxy,stability_proxy,n_events,gate_bridge_tradable,source_path",
                f"{run_id},vol_shock_relaxation,c1,PROMOTED_RESEARCH,1.2,0.1,0.8,210,1,x",
            ]
        )
        + "\n",
        encoding="utf-8",
